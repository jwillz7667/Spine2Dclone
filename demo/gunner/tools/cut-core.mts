// Deterministic, dependency-light sprite-sheet cutter core for the Diamond Fusion asset-prep stage.
...
export function removeBackground(
  ...
  interior?: {
    readonly seeds: ReadonlyArray<{ readonly x: number; readonly y: number }>;
  ...
  if (interior) {
    // One flood per seed (a 3-column reel window needs a seed inside each opening; a seed landing on a
    // divider fails the light/saturation gate and no-ops harmlessly).
    for (const seed of interior.seeds) {
      floodInterior(img, interiorMask, bg, seed.x, seed.y, interior.minLight, interior.maxSat);
    }
  }
...
// Full labelling projection (components + pixel->component map) for callers that need masked crops.
export function labelComponentsWithMap(
  width: number,
  height: number,
  foreground: Uint8Array,
): { components: Component[]; labels: Int32Array } {
  return labelConnected(width, height, foreground);
}

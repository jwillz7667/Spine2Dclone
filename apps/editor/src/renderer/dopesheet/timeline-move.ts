...
// constraint timeline, the path-constraint timeline, and the physics-constraint timeline. These keys are
  ...
  for (const [constraintId, keys] of animation.physics) {
    for (const kf of keys) {
      index.set(kf.id, {
        id: kf.id,
        originTime: kf.time,
        make: (newTime) => new MovePhysicsKeyframeCommand(animId, constraintId, kf.id, newTime),
      });
    }
  }

...
describe('version gate', () => {
  ...
  it('rejects a strictly newer version (major or patch) as UNSUPPORTED_FORMAT_VERSION at /formatVersion', () => {
    ...
    // A newer patch within the same minor is still strictly newer than 0.5.0, so it is unsupported.
    expect(
      errorCodes(validateDocument({ ...cloneMinimal(), hash: '', formatVersion: '0.5.1' })),
    ).toContain('UNSUPPORTED_FORMAT_VERSION');
  });

  it('forward-migrates a below-current 0.1.x document instead of rejecting it (ADR-0004, ADR-0008, ADR-0011)', () => {
    // A 0.1.x document is below the current migration key; the gate runs the chain and the upgraded
    // document validates. cloneMinimal() is already 0.5.0-shaped, so labelling it 0.1.0 with an empty

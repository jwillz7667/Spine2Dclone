...
import { combineTwoColor, type StraightColor } from '@marionette/runtime-web';

// The shared two-color (light + dark) parity vectors. This table is CHARACTER-FOR-CHARACTER the same as
// packages/render-preview/test/two-color.test.ts (TWO_COLOR_VECTORS): the CPU rasterizer (render-preview)
// and this GPU renderer's shared math module must produce identical output for identical inputs, so the two
// renderers cannot drift on the two-color formula. Editing a number here without editing the render-preview
// copy is exactly the divergence this duplicated table is meant to catch. (The packages cannot import each
// other's test code, so the parity contract is the identical numeric table asserted in both suites.)
interface Vector {
  readonly name: string;
  readonly texel: StraightColor;
  readonly light: StraightColor;
  readonly dark: StraightColor;
...
describe('combineTwoColor (runtime-web, parity with render-preview)', () => {
  ...
  it('a black dark term is byte-identical to the single-color light multiply', () => {
    const texel: StraightColor = { r: 0.31, g: 0.62, b: 0.17, a: 1 };
    const light: StraightColor = { r: 0.8, g: 0.5, b: 0.2, a: 1 };

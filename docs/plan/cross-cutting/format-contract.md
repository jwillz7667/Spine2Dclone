...
| `ikConstraints`, `transformConstraints`, `pathConstraints` | May be empty. Constraint names unique across ALL THREE arrays combined (a single constraint namespace). Stage F3 (ADR-0011) adds the required `pathConstraints` array. |
...
- Every constraint (IK, transform, and path) may carry an optional `order` (a non-negative integer). When ANY carries it, ALL must, and the values are a dense unique permutation of `[0, N)` over the COMBINED constraint set spanning all three arrays (`CONSTRAINT_ORDER_INVALID`). Absent `order` means the default order: all IK constraints, then all transform constraints, then all path constraints, each in array order.
...
- Named subtree boundaries: the top-level collections (`/bones`, `/slots`, `/skins`, `/ikConstraints`, `/transformConstraints`, `/pathConstraints`, `/events`, `/animations`, `/atlas`) are the coarse independent subtrees. A check whose entire read set lies under one of them is, by the overlap rule, unaffected by a shape failure under a different one. This is what "independent subtrees where safe" means concretely: independence is read-set disjointness, and "safe" is the no-overlap condition.

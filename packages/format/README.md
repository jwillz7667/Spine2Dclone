...
|---|---|---|
| `CURRENT_FORMAT_VERSION` | `0.4.0` | `SkeletonDocument` (`formatVersion` field) |
...
- **`SkeletonDocument`** (`src/schema/document.ts`, strict): `formatVersion`, `name`, `hash`,
  ...
  and an optional `metadata` block. IK constraints carry depth (softness/stretch/compress/uniform, signed
  bend) and both constraint arrays share an optional explicit `order`; transform constraints carry
  `local`/`relative` variants. Attachments include the six kinds (region, mesh, linked mesh, clipping,
  point, boundingbox) with optional frame sequences on region/mesh; skins may scope bones and constraints.
  Each animation carries bone/slot/ik/transform/deform timelines (bone tracks joint or per-component, slot
  color joint or split rgb/alpha plus a two-color dark and a sequence track) plus the `drawOrder` and
  `events` timelines. The core skeletal-animation format (Layer A of the product).
...
- `migrateToCurrent` / `runMigrations` / `MIGRATIONS`: the migration framework. The registry holds three
  ...
  timelines), and `0.3.x -> 0.4.0` (maps `bendPositive` to the signed `bend` and injects the IK depth and
  transform variant defaults), each recomputing the hash. The runner walks the contiguous chain and
...
single `FORMAT_ERROR_CODES` const array (65 codes today, for example `SCHEMA_SHAPE`,
...
`CONSTRAINT_ORDER_INVALID`, `LINKED_MESH_CYCLE`, `TIMELINE_COMPONENT_CONFLICT`, `HASH_MISMATCH`);
...
pnpm --filter @marionette/format test                 # vitest (28 test files)

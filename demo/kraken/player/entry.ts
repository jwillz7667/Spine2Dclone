...
// The Kraken's Hoard standalone player, v2: a PLAYABLE spin loop on top of the REAL @marionette/runtime-web
// renderer. Presentation only, ZERO document mutation (Law 1/2). The SkeletonView renders the MCP-authored
// document (its setup pose IS the crafted landing board); the spin is layered on with pieces the player
// owns: a per-column strip of region sprites that scrolls + wraps procedurally while a reel is spinning,
// and AnimationState tracks that play the authored spin-loop animations (reel_stop_bounce, anticipation_glow,
// scatter_land, bonus_intro) live. When a reel stops the player hides that column's strip, revealing the
// skeleton cells beneath (already the outcome), and plays the bounce. Particles are the runtime-core
// EffectSystem solved live and drawn through fillEmitterBatch into pooled sprites. Everything is inlined in
// one HTML file (document JSON, effects JSON, atlas pages as data URLs, this bundle); it runs from file://.
...
const ROW_PITCH = ROWS[1]! - ROWS[0]!; // 180: the cell-to-cell vertical spacing
const REEL_STOP_STAGGER_MS = 280;
const GRID_CENTER = { x: 0, y: ROWS[1]! };
// The full symbol vocabulary the scrolling strips cycle through (mirrors author-game.mts).
const SYMBOLS = [
  'blue_ruby',
  'green_ruby',
  'purple_ruby',
...
const SPIN_BASE_MS = 650; // full-speed spin before the first reel stops
...
const STRIP_TOP = ROWS[0]! - ROW_PITCH; // first sprite's y; the strip extends one pitch past each edge
const WRAP_LIMIT = ROWS[2]! + ROW_PITCH; // a sprite past this wraps back to the top
...
async function main(): Promise<void> {
  ...
  // The three world-space layers the player composites, all sharing one SCENE->screen transform: the
  // skeleton (the game board), the reel strips (opaque, on top, revealed away as reels stop), and the
  // particles (on top of everything, the celebration).
  ...
  // One scrolling strip per reel column. Each is a masked container at the column's world x holding a dark
  // backing (so the static cells beneath are hidden while spinning) plus STRIP_COUNT symbol sprites.
  const reels: Reel[] = [];
  for (let c = 0; c < COLS.length; c += 1) {
    ...
    backing
      .rect(-CELL / 2 - 8, WINDOW_TOP, CELL + 16, WINDOW_BOTTOM - WINDOW_TOP)
      .fill({ color: 0x02090d, alpha: 1 });
    ...
    mask.rect(-CELL / 2 - 8, WINDOW_TOP, CELL + 16, WINDOW_BOTTOM - WINDOW_TOP).fill(0xffffff);
  ...
  vignette
    .rect(
      COLS[3]! - CELL / 2 - 8,
      WINDOW_TOP,
      COLS[4]! - COLS[3]! + CELL + 16,
      WINDOW_BOTTOM - WINDOW_TOP,
    )
    .fill({ color: 0xffcf4d, alpha: 1 });
  ...
  // Draw one solved effect frame: for each emitter view, fill the shared batch (world position via the
  // anchor, rotation/scale/tint/alpha) and push it into pooled sprites; hide any pooled sprite past the
  // live count. Mirrors render-preview's gatherEffectDrawItems against the SAME fillEmitterBatch bridge.
  ...
  // AnimationState tracks: 0 = base (idle / win / scatter_land / bonus_intro), 1 = additive reel bounce,
  // 2 = replace anticipation glow on the landed scatter cells.
  ...
  // Celebrate stays: a standalone crossfade to the win animation and back (unchanged behavior).
  ...
  app.ticker.add((ticker) => {
    ...
    if (vignette.visible) {
      vignetteTime += dt;
      vignette.alpha = 0.1 + 0.14 * Math.abs(Math.sin(vignetteTime * 3.2));
    }

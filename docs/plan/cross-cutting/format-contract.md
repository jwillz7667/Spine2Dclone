...
| `ikConstraints`, `transformConstraints`, `pathConstraints`, `physicsConstraints` | May be empty. Constraint names unique across ALL FOUR arrays combined (a single constraint namespace). Stage F3 (ADR-0011) adds the required `pathConstraints` array; stage F4 (ADR-0014) adds the required `physicsConstraints` array. |
...
| `physics` (optional) | The optional skeleton-level physics settings block (stage F4, ADR-0014): `gravity`, `wind` (unbounded finite world defaults), and `mix` (`[0, 1]` master fader, `PHYSICS_MIX_RANGE`). Absent means the identity defaults (no global weather, unit master mix); the migration injects nothing for it. |
...
### 4.6 The seven attachment kinds

`Attachment` is a discriminated union on `type` (`region | mesh | linkedmesh | clipping | point | boundingbox | path`). Zod uses `z.discriminatedUnion('type', [...])`. Stage F2 (ADR-0009) added the closed `linkedmesh` kind and an optional frame `sequence` block on `region` and `mesh`; stage F3 (ADR-0011) added the closed `path` kind.
...
| `path` | A piecewise cubic Bezier spline (stage F3, ADR-0011). No atlas reference and no color (pure geometry, like `boundingbox`). `closed`/`constantSpeed` booleans; `lengths` is the cumulative per-curve arc-length table (length equals the derived curve count, non-negative non-decreasing: `PATH_LENGTHS_COUNT`, `PATH_LENGTHS_ORDER`); `vertices` are the control points in the section 6 weighted/unweighted vertex codec (the shared `MESH_*` vertex-stream codes), with the control-point count valid for a closed/open cubic spline (`PATH_VERTEX_COUNT`). |
...
- `PathConstraint` (stage F3, ADR-0011): `bones` non-empty (`PATH_BONES_EMPTY`), all exist (`PATH_BONE_MISSING`). `target` names an existing SLOT (`PATH_TARGET_MISSING`, not a bone: a path lives on a slot); when the target slot's setup attachment is statically decidable it must be a `path` (`PATH_TARGET_NOT_PATH`). `positionMode`/`spacingMode`/`rotateMode` are closed enums; `position`/`spacing`/`offsetRotation` are unbounded finite; `mixRotate`/`mixX`/`mixY` in `[0, 1]` (`PATH_MIX_RANGE`).
- `PhysicsConstraint` (stage F4, ADR-0014): `bone` names a single existing bone (`PHYSICS_BONE_MISSING`) that is BOTH the driven bone and its own setpoint reference (no external target). `channels` is a non-empty (`PHYSICS_CHANNELS_EMPTY`), duplicate-free (`PHYSICS_CHANNEL_DUPLICATE`) subset of `x`/`y`/`rotation`/`scaleX`/`shearX`. `step` is `> 0` (`PHYSICS_STEP_RANGE`, the fixed simulation timestep); `inertia`/`damping`/`mix` in `[0, 1]` (`PHYSICS_INERTIA_RANGE`/`PHYSICS_DAMPING_RANGE`/`PHYSICS_MIX_RANGE`); `strength` `>= 0` (`PHYSICS_STRENGTH_RANGE`); `mass` `> 0` (`PHYSICS_MASS_RANGE`); `wind`/`gravity` unbounded finite. The physics SOLVE (a deterministic fixed-step damped spring) is runtime work (ADR-0014, Lane B); the format carries the shape, the reference validity, and the settings block.
- Every constraint (IK, transform, path, and physics) may carry an optional `order` (a non-negative integer). When ANY carries it, ALL must, and the values are a dense unique permutation of `[0, N)` over the COMBINED constraint set spanning all four arrays (`CONSTRAINT_ORDER_INVALID`). Absent `order` means the default order: all IK constraints, then all transform constraints, then all path constraints, then all physics constraints, each in array order.
- Constraints are solved IK then transform then path (the default order), before world transforms. The canonical per-frame solve order is handoff section 6; its behavior is owned by `runtime-core` and locked by conformance, NOT by this document. The F2 depth/variant/order fields carry solve capability owned by `runtime-core` (PP-B5); the format validates only their shape and the `order` permutation. Constraint parameters are animatable via the `ik` and `transform` timelines.
...
- `ik`, `transform`, `path`, and `physics` timelines key constraint parameters by constraint name; the name must reference an existing constraint of that kind (`ANIM_IK_UNKNOWN`, `ANIM_TRANSFORM_UNKNOWN`, `ANIM_PATH_UNKNOWN`, `ANIM_PHYSICS_UNKNOWN`). A `PathFrame` (stage F3, ADR-0011) is partial and may carry `position`/`spacing` (unbounded finite) and `mixRotate`/`mixX`/`mixY` (range-checked to `[0, 1]`, `PATH_MIX_RANGE`). A `PhysicsFrame` (stage F4, ADR-0014) is partial and may carry `mix`/`inertia`/`damping` (range-checked to `[0, 1]`), `strength` (`>= 0`), and `wind`/`gravity` (unbounded finite), reusing the constraint-definition `PHYSICS_*_RANGE` refinements; `step`, `mass`, and `channels` are structural and NOT keyable. `IkFrame.mix` and every `TransformFrame.mix*` channel PRESENT in a frame is range-checked to `[0, 1]` (`IK_MIX_RANGE`, `TC_MIX_RANGE`), the same refinement applied to the constraint definitions in section 4.7. Stage F2 (ADR-0009) keys the signed `IkFrame.bend` (superseding the pre-0.4.0 `bendPositive`) with optional `softness`/`stretch`/`compress` depth channels; `bend` is sampled stepped (non-interpolatable). A `TransformFrame` MAY carry a subset of mix channels (the frame type is partial). The MEANING of an absent channel during a frame (hold, zero, or inherit) is a SOLVE-SEMANTICS decision owned by `runtime-core` and locked by conformance; the format assigns it no value and does not adjudicate it.
...
- Named subtree boundaries: the top-level collections (`/bones`, `/slots`, `/skins`, `/ikConstraints`, `/transformConstraints`, `/pathConstraints`, `/physicsConstraints`, `/events`, `/animations`, `/atlas`) are the coarse independent subtrees. A check whose entire read set lies under one of them is, by the overlap rule, unaffected by a shape failure under a different one. This is what "independent subtrees where safe" means concretely: independence is read-set disjointness, and "safe" is the no-overlap condition.

...
> This module lives at `packages/document-core/` (amended by ADR-0001; was `apps/editor/src/renderer/document/`). It is renderer-agnostic (imports only `@marionette/format`, plus `@marionette/runtime-core` for transform commands; no React, PixiJS, DOM, Electron, or Node), so the editor renderer AND a headless MCP server in the Electron main process drive the SAME commands (user + AI control, Law 2). The editor's composition root, stores, tools, gizmo, keybindings, and document host stay under `apps/editor/src/renderer/`. References to `document/model/**`, `document/commands/**`, `document/history/**` below map to `packages/document-core/src/model/**`, `.../commands/**`, `.../command/** (history)`. It is built in Phase 0 and is a hard dependency of every later phase.

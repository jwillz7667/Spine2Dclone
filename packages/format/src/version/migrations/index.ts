...
// Inject the stage F3 (ADR-0011) `path` timeline into each animation, preserving any existing content. A
// 0.4.x animation has { duration, bones, slots, ik, transform, deform, drawOrder, events }; the empty
// record makes it a valid 0.5.0 animation. Pure: returns a new record, never mutates the input.
function migrate04xAnimations(animations: Record<string, unknown>): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const [name, anim] of Object.entries(animations)) {
    if (isRecord(anim)) {
      out[name] = {
        ...anim,
        path: isRecord(anim['path']) ? anim['path'] : {},
      };
    } else {
      out[name] = anim;
    }
...
// 0.4.x -> 0.5.0 (ADR-0011): add the root `pathConstraints` array and the per-animation `path` timeline,
// stamp formatVersion 0.5.0, and recompute the content hash when the source carried one (a draft with an
// empty hash stays a draft). The injected content changes the canonical bytes, so a non-empty hash MUST be
// recomputed or the load path's hash layer would reject the migrated document. The path ATTACHMENT kind and
// the path CONSTRAINT fields are new-and-unreferenced by a 0.4.0 document, so nothing else is injected.
function migrate04xTo05(input: unknown): unknown {
  if (!isRecord(input)) return input;
  const animations = isRecord(input['animations'])
    ? migrate04xAnimations(input['animations'])
    : input['animations'];
  const next: Record<string, unknown> = {
    ...input,
    pathConstraints: Array.isArray(input['pathConstraints']) ? input['pathConstraints'] : [],
    animations,
    formatVersion: '0.5.0',
  };
  const sourceHash = input['hash'];
